    ...
    <div className="flex h-full flex-col rounded-2xl border border-zinc-800 bg-surface p-3">
      <div className="flex items-center justify-between">
        ...
        <span className="inline-flex items-center gap-1.5 text-xs">
          <span className="h-3 w-1 rounded-full" style={{ backgroundColor: row?.team_colour ?? '#3f3f46' }} />
          <span className="font-semibold text-zinc-200">{row ? row.abbreviation ?? row.driver_number : '-'}</span>
        </span>
      </div>
      <div className="mt-2 flex min-h-0 flex-1 flex-col justify-center gap-3">
        ...
        <Bar label="Throttle" value={row?.throttle ?? null} color="bg-emerald-500" />
        <Bar label="Brake" value={row?.brake ?? null} color="bg-f1-red" />
      </div>
    </div>
  )
}
